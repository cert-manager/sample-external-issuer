...
release: helm-chart | $(NEEDS_CRANE)
	$(eval helm_templated_yaml := $(bin_dir)/scratch/$(helm_chart_name)-$(helm_chart_version).yaml)
	$(HELM) template sample-external-issuer "$(helm_chart_archive)" --set omitHelmLabels=true > $(helm_templated_yaml)
	...
	@echo "RELEASE_YAML=$(helm_templated_yaml)" >> "$(GITHUB_OUTPUT)"
